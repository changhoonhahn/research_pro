pro build_envcount_smf, run, nolit=nolit, twoenv=twoenv
...
; export final data
                smfdata_file = get_path(/envsmfdata)+'smf_'+survey+'_cylr'+rad_string+'h'+h_string+$
                    '_thresh'+thresh_string+litsuffix+'_'+strtrim(sf_q[i])+'_'+$
                    envbin[ee]+'_'+zbin[iz]+'z_'+strtrim(string(n_elements(envbin)),2)+'envbin.fits'
                mwrfits, finaldata, smfdata_file, /create

; generate the SMFs using im_mf_vmax.pro 
...
; compute the cosmic variance errors
                ...
                if (survey EQ 'sdss') then begin
                    for bb=0L,mfdata.nbins-1L do begin 
                        if (rev[bb] ne rev[bb+1]) then begin 
                            mfdata.nfield = 1
                            mfdata.thesefields[0,bb] = 'sdss'
                        endif 
                    endfor
                endif else begin 
                   for bb=0L,mfdata.nbins-1L do begin 
                       if (rev[bb] ne rev[bb+1]) then begin 
                           allfield = strtrim((finaldata.field)[rev[rev[bb]:rev[bb+1]-1]],2)
                ...
                fixx = where(mfdata.nfield gt 0 and mfdata.phierr_cv le 0.0,nfix)
                if (nfix ne 0) then begin 
                    get_element, good, fixx, nearest
                    mfdata.phierr_cv[fixx] = mfdata.phierr_cv[good[nearest]]
                endif 
                
                fname='smf_'+survey+'_cylr'+rad_string+'h'+h_string+$
                    '_thresh'+thresh_string+litsuffix+'_'+strtrim(sf_q[i])+'_'+$
