...
redshift = ["$0.6<z<0.8$","$0.4<z<0.6$","$0.2<z<0.4$","$0.0375<z<0.145$"]
color = ['b','m','r','g']
...
for i in range(len(sfq)):
    for ii in range(len(ebin)):
        ...
        for iii in range(len(zbin)):
            fname = 'smf_primus_cylr'+rad+'h'+height+'_thresh'+thresh+'_lit_'+sfq[i]+'_'+ebin[ii]+'_'+zbin[iii]+'_bin'+binsize+envbinsuffix+'.fits'
            if zbin[iii]=='0002z': 
                fname = 'smf_sdss_cylr'+sdss_rad+'h'+sdss_height+'_thresh'+sdss_thresh+'_'+sfq[i]+'_'+ebin[ii]+'_nobinz_bin'+binsize+envbinsuffix+'.fits'
            print fname
...
#        qfmass = massdata
        ...
        qf_fname = 'qf_primus_cylr'+rad+'h'+height+'_thresh'+thresh+'_lit_'+ebin[j]+'_'+zbin[jj]+'_bin'+binsize+envbinsuffix+'.fits'
        if zbin[jj]=='0002z':
            qf_fname = 'qf_sdss_cylr'+sdss_rad+'h'+sdss_height+'_thresh'+sdss_thresh+'_'+ebin[j]+'_nobinz_bin'+binsize+envbinsuffix+'.fits'
        print qf_fname
        ...
        if zbin[jj]=='0002z': 
            qfzero = (qflimit==1) & (qfqf > 0.0) & (qfqf < 1.0) & (qfmass > 9.5)# & (qfmass < 11.0) 
        ...
        ax1.errorbar(qfmass[(qflimit==1) & (qfqf > 0.0) & (qfqf < 1.0)],qfqf[(qflimit==1) & (qfqf > 0.0) & (qfqf < 1.0)],yerr=qferrcv[(qflimit==1) & (qfqf > 0.0) & (qfqf < 1.0)],fmt=color[jj]+'o', capsize=3, elinewidth=1)
...
fig.text(0.5,0.01,r"$\bf{log(M/M_{\odot})}$", ha='center', va='center', fontsize=18)
fig.text(0.03,0.66,r"$\bf{log(\Phi / Mpc^{-3} dex^{-1})}$", ha='center', va='center', rotation='vertical', fontsize=18)
qffitfig.text(0.5, 0.95, r'Radius '+str(rad)+' Height '+str(height), ha='center', va='center', fontsize=20)
