...
; channel, and also be sure to check for non-detections
    ...
    if (ngood ne 0L) then begin
       ...
       ebv[good] = dust_getval_chh(gl,gb,/interp,/noloop)
    endif
...
; now correct for extinction, convert to ivarmaggies, and return
    ...
    if (nngood ne 0L) then begin
       ...
       for ii=0L,nngood-1L do begin 
           if (obsmaggieserr[1,ngood[ii]]*factor[ii] LE 0.0) then $ 
               ivarmaggies[1,ngood[ii]] = 0.0 else $
               ivarmaggies[1,ngood[ii]] = 1.0/(obsmaggieserr[1,ngood[ii]]*factor[ii])^2.0
       endfor
    endif
    ...
    if (nfgood ne 0L) then begin
       ...
       for ii=0L,nfgood-1L do begin 
           if (obsmaggieserr[0,fgood[ii]]*factor[ii] LE 0.0) THEN $
              ivarmaggies[0,fgood[ii]] = 0.0 else $
              ivarmaggies[0,fgood[ii]] = 1.0/(obsmaggieserr[0,fgood[ii]]*factor[ii])^2.0
      endfor
    endif
